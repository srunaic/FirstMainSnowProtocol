...
namespace YeongYeon
{
    public class PlayerController : MonoBehaviour, IPunObservable
    {
        ...
        [SerializeField]
        public float runSpeed = 30f; // 달리는 속도

        private float nowSpeed;//실제속도
        private float targetSpeed;//목표속도
        ...
        public void RunariaMove()
        {
            ...
            // 이동량이 0보다 큰지 확인하여 움직일때만 플레이어를 회전하려고 합니다.
            #endregion
            ...
            Vector3 velocity = inputVec * nowSpeed;
                velocity = Quaternion.LookRotation(new Vector3(Camera.main.transform.forward.x, 0f, 
                Camera.main.transform.forward.z)) * velocity;
                ...
                if (Input.GetButtonDown("Jump"))
                {
                    ...
                    if (Input.GetButtonUp("Jump") && downward_velocity > 0f)
                    {
                        downward_velocity *= 0.5f; //점프 속도가 절반으로 줄어든다
                                                   ...
                                                   //더 많은 선택 의지와 움직임의 정확성을 제공하여 게임 플레이를 향상 시킬 수 있다.
                    }
